...
def log(log_type, msg=None, asset=None, new_asset=None, request=None):
    ...
    if log_type == "upline":
        ...
        event.name = "%s <%s>: 数据更新成功" %(asset.asset_type, asset.sn)
        event.asset = asset
        event.detail = "资产更新成功!"
...
class UpdateAsset:
    ...
    def __init__(self, request, asset, report_data):
        self.request = request
        self.asset = asset
        self.report_data = report_data                     # 客户端发送过来的整个数据字符串
        self.asset_update()
    ...
    def _update_manufacturer(self):                     #更新厂商
        m = self.report_data.get("manufacturer")
        if m:
            manufacturer_obj, _ = models.Manufacturer.objects.get_or_create(name=m)    # 实例化  name是manufacturer的属性（字段
            self.asset.manufacturer = manufacturer_obj
        else:
            self.asset.manufacturer = None
        self.asset.manufacturer.save()

    def _update_server(self):                            # 更新服务器
        self.asset.server.model = self.report_data.get('model')                   # model 均是属性
        self.asset.server.os_type = self.report_data.get('os_type')                 # sys_info collect
    ...
    def _update_RAM(self):
        ...
        new_rams_list = self.report_data['ram']                # 获取新数据中的内存信息，并转成字典格式
        new_rams_dict = dict()
        if new_rams_list:
        ...
        if new_rams_dict:
            for key in new_rams_dict:
                defaults = {
                    'sn': new_rams_dict[key].get('sn'),                      # 更新key
