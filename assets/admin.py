...
class NewAssetAdmin(admin.ModelAdmin):
    ...
    def approve_selected_new_assets(self, request, queryset):
        selected = request.POST.getlist(admin.ACTION_CHECKBOX_NAME)   # 获取被打钩的checkbox对应的资产
        success_upline_number = 0
        for asset_id in selected:
            obj = asset_handler.ApproveAsset(request, asset_id)      # 每个资产建一个实例
            ret = obj.asset_upline()                                 # 调用asset_upline
            if ret:
